...
import { Grid, Container, Box, Typography, Accordion, AccordionSummary, AccordionDetails, Icon } from '@material-ui/core'
...
const Departments = () => {
    ...
    let dataObj = {};
    if(infoObj) {
        dataObj = departmentData.find(item=>(item.deptName == infoObj.title +' Department'));
        if(!dataObj) dataObj = {}
    }else {
            infoObj = {};
    }
        ...
        <Layout>
            <Container className={classes.root}>
                ...
                <Grid container spacing={2}>
                    ...
                    {
                        dataObj&&dataObj.teamLeads?dataObj.teamLeads.map((item, i)=>{
                            return <Box className={classes.flex} key={i}>
                                <img className={classes.circle} width="40%" src={item.src} alt="logo"/>
                                <p>{item.name || ''}</p>
                            </Box>
                        }) : ''
                    }
                        ...
                        dataObj&&dataObj.projects?dataObj.projects.map((item, i)=>{
                            return <Accordion className={classes.accordion} expanded={expanded[i]} onChange={handleChange(i)} key={i}>
                                        ...
                                        <Grid container spacing={2}>
                                            ...
                                            <Grid item xs={6}>
                                                {item&&item.imageURLs&&item.imageURLs[0]?<img width="100%" src={item.imageURLs[0]} />:''}
                                            </Grid>
                                        </Grid>
                                        </AccordionDetails>
                                </Accordion>
